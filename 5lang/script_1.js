(() => {
  ...
  function makeReportDocx(){
    ...
    let imageId=1;
    const brandingTable=()=>{
      const widths=[720,7305,2675], grid=widths.map(w=>`<w:gridCol w:w="${w}"/>`).join('');
      ...
      const title=reportDocxCell(reportDocxParagraph(reportDocxRun(String(L.reportTitle).toUpperCase(),22,'1C1C1E',true),{line:560,keep:true,align:'right'}),widths[2],{top:60,bottom:80,left:0,right:0,bottomBorder:'18',bottomBorderColor:'1C1C1E'});
